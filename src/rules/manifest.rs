...
const CARGO_SEEDS: &[CrateEntry] = &[
    ...
    CrateEntry { name: "rsa", crypto_algorithm: Some("RSA"), confidence: 0.9 },
    CrateEntry { name: "p256", crypto_algorithm: Some("ECDSA"), confidence: 0.9 },
    CrateEntry { name: "p384", crypto_algorithm: Some("ECDSA"), confidence: 0.9 },
    CrateEntry { name: "ed25519-dalek", crypto_algorithm: Some("Ed25519"), confidence: 0.9 },
    CrateEntry { name: "x25519-dalek", crypto_algorithm: Some("X25519"), confidence: 0.9 },
    CrateEntry { name: "ecdsa", crypto_algorithm: Some("ECDSA"), confidence: 0.9 },
    // Tier 2 — confidence 0.6, mixed algorithms
    CrateEntry { name: "ring", crypto_algorithm: None, confidence: 0.6 },
    CrateEntry { name: "openssl-sys", crypto_algorithm: None, confidence: 0.6 },
    CrateEntry { name: "aws-lc-rs", crypto_algorithm: None, confidence: 0.6 },
];
...
const PIP_PACKAGES: &[PipEntry] = &[
    PipEntry { name: "python-rsa", crypto_algorithm: Some("RSA"), confidence: 0.95 },
    PipEntry { name: "rsa", crypto_algorithm: Some("RSA"), confidence: 0.95 },
    PipEntry { name: "ecdsa", crypto_algorithm: Some("ECDSA"), confidence: 0.95 },
    PipEntry { name: "ed25519", crypto_algorithm: Some("Ed25519"), confidence: 0.95 },
    PipEntry { name: "pynacl", crypto_algorithm: Some("Ed25519"), confidence: 0.9 },
    PipEntry { name: "paramiko", crypto_algorithm: Some("RSA"), confidence: 0.8 },
    PipEntry { name: "fabric", crypto_algorithm: Some("RSA"), confidence: 0.7 },
    PipEntry { name: "cryptography", crypto_algorithm: None, confidence: 0.5 },
    PipEntry { name: "pyopenssl", crypto_algorithm: None, confidence: 0.5 },
    PipEntry { name: "pycryptodome", crypto_algorithm: None, confidence: 0.5 },
    PipEntry { name: "pycryptodomex", crypto_algorithm: None, confidence: 0.5 },
];
...
impl Rule for CargoLockPqCrypto {
    ...
    fn cwe(&self) -> Option<&str> { Some("CWE-327") }
    fn description(&self) -> &str { "Dependency uses quantum-vulnerable cryptographic algorithm" }
    fn language(&self) -> Language { Language::Manifest }
    fn cnsa2_deadline(&self) -> Option<&'static str> { Some("2033") }
    ...
    fn check(&self, source: &str, _tree: &tree_sitter::Tree) -> Vec<Finding> {
        ...
        for (i, pkg) in packages.iter().enumerate() {
            ...
            let snippet_pattern = format!("name = \"{}\"", pkg_name);
            let offset = source.find(&snippet_pattern).unwrap_or(0);

            // Extend to include version line
            ...
            let end = source
                .find(&full_pattern)
                .map(|s| s + full_pattern.len())
                .unwrap_or(offset + snippet_pattern.len());
...
impl Rule for RequirementsTxtPqCrypto {
    fn id(&self) -> &str { "manifest/pip-pq-vulnerable-dep" }
    fn severity(&self) -> Severity { Severity::High }
    fn cwe(&self) -> Option<&str> { Some("CWE-327") }
    fn description(&self) -> &str { "Dependency uses quantum-vulnerable cryptographic algorithm" }
    fn language(&self) -> Language { Language::Manifest }
    ...
    fn check(&self, source: &str, _tree: &tree_sitter::Tree) -> Vec<Finding> {
        let pip_map: HashMap<&str, &PipEntry> =
            PIP_PACKAGES.iter().map(|e| (e.name, e)).collect();
        ...
        for line in source.lines() {
            ...
            let lookup = pkg_name.to_lowercase().replace('_', "-").replace('.', "-");
...
mod tests {
    ...
    fn extract_pip_name_with_dots_and_underscores() {
        assert_eq!(extract_pip_package_name("my.package_name>=1.0"), "my.package_name");
    }
}
